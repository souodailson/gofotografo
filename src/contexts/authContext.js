...
const isInvalidTokenError = (error) => {
  return error && (error.message.includes("Invalid Refresh Token") || error.message.includes("Token not found") || error.message.includes("Session not found") || (error.code === 400 && error.error_code === "refresh_token_not_found"));
};

export const AuthProvider = ({ children }) => {
  const { toast, isMobileView } = useToast();
  ...
  const handleError = useCallback(async (error, context) => {
    if (isInvalidTokenError(error)) {
      toast({
        ...
        description: "Sua sessão expirou ou tornou-se inválida. Por favor, faça login novamente.",
        variant: "destructive",
        duration: 7000,
      });
      if (supabase) {
  ...
  useEffect(() => {
    ...
    const getSessionAndUser = async () => {
      ...
      if (activeUser && activeUser.email_confirmed_at) {
        ...
        setSettingsData(defaultInitialSettings);
        setLoadingSettings(false);
        if (activeUser && !activeUser.email_confirmed_at && !isMobileView) {
          toast({
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      async (event, newSession) => {
        ...
        if (currentUser && !currentUser.email_confirmed_at && event !== 'INITIAL_SESSION' && !isMobileView) {
          toast({
             title: "Email não confirmado",
             description: "Por favor, confirme seu email para continuar.",
             variant: "destructive",
  ...
  }, [handleError, fetchSettings, toast, isMobileView]);
